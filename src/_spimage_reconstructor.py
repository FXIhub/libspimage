...
# Set up logger
logger = logging.getLogger('RECONSTRUCTOR') # pylint: disable=invalid-name
...
class Reconstructor:
    ...
    def _get_scores(self):
        ...
        out["real_error"] = np.sqrt((abs(I[M == 0])**2).sum() /
                                    ((abs(I[M == 1])**2).sum() + np.finfo("float32").eps))
        out["fourier_error"] = np.sqrt((abs(abs(fI[fM == 1]) - abs(np.fft.fftshift(self._sp_amplitudes.image)[fM == 1]))**2).sum() /
                                       ((abs(np.fft.fftshift(self._sp_amplitudes.image)[fM == 1])**2).sum() + np.finfo("float32").eps))
        out["FcFo"] = np.sqrt(abs(fI[fM == 1]).sum() /
                              (abs(self._sp_amplitudes.image[fM == 0]).sum() + np.finfo("float32").eps))
        out["support_size"] = M.sum() / float(M.shape[0] * M.shape[1])
        return out

    @staticmethod
    def _log(msg, mode="INFO"):
    ...
    def set_intensities(self, intensities, shifted=False):
        ...
        By default it is expected that the provided image is the non-shifted version of the \
        diffraction pattern given as a 2D numpy array (diffraction pattern representation). \
        If desired change to \"shifted=False\". If \"shifted=True\" pixel (0, 0) is located \
        in the corner of the physical diffraction pattern.
        """
        self._amplitudes_dirty = True
        self._initial_support_dirty = True
    ...
    def set_mask(self, mask, shifted=False):
        """
        The mask is a 2D boolean numpy array with the same shape as the provided intensities.

        Values that equal True indicate valid pixels and values that equal False indicate unknown \
        intensity values at the respective pixel location. By default it is expected that the \
        provided image is the non-shifted version of the diffraction pattern given as a 2D numpy \
        array (diffraction pattern representation). If desired change to \"shifted=False\". If \
        \"shifted=True\" pixel (0, 0) is located in the corner of the physical diffraction pattern.
        """
        self._amplitudes_dirty = True
        self._initial_support_dirty = True
    ...
    def set_number_of_outputs_images(self, number_of_outputs_images):
        """
        Specify the number of times that images (such as real space and fourier space and support \
        images etc.) shall be written to the output. The output interval will be calculated from \
        the number of iterations specified by set_number_of_iterations.
        """
    ...
    def set_number_of_outputs_scores(self, number_of_outputs_scores):
        """
        Specify the number of times that scores (such as real space error and fourier space error \
        and support size etc.) shall be written to the output. The output interval will be \
        calculated from the number of iterations specified by set_number_of_iterations.
        """
    ...
    def set_number_of_outputs(self, number_of_outputs):
        """
        Specify the number of times that scores (such as real space error and fourier space error \
        and support size etc.) and images (such as real space error and fourier space error and \
        support size etc.) shall be written to the output. The output interval will be calculated \
        from the number of iterations specified by set_number_of_iterations. If you like to \
        specify the number of outputs of images and scores separately call the functions \
        set_number_of_outputs_scores and set_number_of_outputs_images.
        """
    ...
    def set_initial_support(self, **kwargs):
        ...
        - an explicit support mask \"support_mask=2D_boolean_numpy_array\", non-shifted and of \
                the same shape as the intensity pattern
        """
        if not ("radius" in kwargs or "support_mask" in kwargs or "area" in kwargs):
            self._log("set_initial_support requires one of the following key word arguments: \
                       radius, area, support_mask", "ERROR")
            return
    ...
    def set_support_algorithm(self, atype, **kwargs):
        """
        ...
        * a gaussian blur is applied with a standard deviation of the kernel with the given blur \
                value in pixel
        ** area as a fraction of the entire image [0.0...1.0]
        NOTE: If you like to use a series of support-update algorithms please use instead the \
        function append_support_algorithm.
        """
        self._clear_support_algorithms()
        kwargs1 = dict(kwargs)
        if "number_of_iterations" not in kwargs:
            kwargs1["number_of_iterations"] = None
        self.append_support_algorithm(atype, **kwargs1)

    def append_support_algorithm(self, atype, **kwargs):
        """
        Append one of the following support-update algorithms with the respective keyord arguments:
        - area: number_of_iterations, update_period, blur_init*, blur_final*, \
                area_init**, area_final**
        - threshold: number_of_iterations, update_period, blur_init*, blur_final*, \
                     threshold_init, threshold_final
        - static: number_of_iterations
        * a gaussian blur is applied with a standard deviation of the kernel with the given blur \
                value in pixel
        ** area as a fraction of the entire image [0.0...1.0]
        NOTE: If you like to use only a single support-update algorithm you might want to use \
        instead the function set_support_algorithm.
        """
        alg_conf = {"type":atype}
        alg_conf["center_image"] = kwargs.get("center_image", True)
        # check input
        if "number_of_iterations" not in kwargs:
            self._log("append_support_algorithm requires the keyword argument \
                       \'number_of_iterations\'.", "ERROR")
            return
        alg_conf["number_of_iterations"] = kwargs["number_of_iterations"]
        if alg_conf["number_of_iterations"] is None and self._support_algorithms_configs:
            self._log("You can not have more than one support algorithm of unspecified number of \
                       iterations if the total number of iterations is set to None. Set the total \
                       number of iterations by calling set_number_of_iterations and try again.",
                      "ERROR")
            return
        necessary_kwargs = {"area":["update_period", "blur_init", "blur_final",
                                    "area_init", "area_final"],
                            "threshold":["update_period", "blur_init", "blur_final",
                                         "threshold_init", "threshold_final"],
                            "static":[]}
        if atype not in necessary_kwargs:
            self._log("append_support_algorithm accepts algorithms of the following types: %s" %
                      str(necessary_kwargs.keys()),
                      "ERROR")
            return
        for k in necessary_kwargs[atype]:
            if k not in kwargs:
                self._log("append_support_algorithm with the support algorithm \"area\" requires \
                           the following argument: " + k, "ERROR")
                return
            alg_conf[k] = kwargs[k]
        if alg_conf["number_of_iterations"] is not None:
            if not self._support_algorithms_configs:
    ...
    def set_phasing_algorithm(self, atype, **kwargs):
        """
        ...
        NOTE: If you like to use a series of phasing algorithms please use instead the function \
        append_phasing_algorithm.
        """
        self._clear_phasing_algorithms()
        kwargs1 = dict(kwargs)
        if "number_of_iterations" not in kwargs:
            kwargs1["number_of_iterations"] = None
        self.append_phasing_algorithm(atype, **kwargs1)

    def append_phasing_algorithm(self, atype, **kwargs):
        """
        ...
        NOTE: If you like to use only a single phasing algorithms you might want to use instead \
        the function set_phasing_algorithm.
        """
        alg_conf = {"type":atype}
        # check input
        if "number_of_iterations" not in kwargs:
            self._log("append_phasing_algorithm requires the keyword argument \
                       \'number_of_iterations\'.", "ERROR")
            return
        alg_conf["number_of_iterations"] = kwargs["number_of_iterations"]
        if alg_conf["number_of_iterations"] is None and self._phasing_algorithms_configs:
            self._log("You can not have more than one phasing algorithm of unspecified number of \
                       iterations if the total number of iterations is set to None. Set the total \
                       number of iterations by calling set_number_of_iterations and try again.",
                      "ERROR")
            return
        ...
        necessary_kwargs = {"raar":["beta_init", "beta_final"],
                            ...
                            "er":[],
                            "diffmap":["beta_init", "beta_final", "gamma1", "gamma2"]}
        if atype not in necessary_kwargs.keys():
            self._log("append_phasing_algorithm accepts algorithms of the following types: %s" %
                      str(necessary_kwargs.keys()), "ERROR")
            return
        for k in necessary_kwargs[atype]:
            if k not in kwargs:
                self._log("append_phasing_algorithm with the phasing algorithm " + atype +
                          " requires the following argument: " + k, "ERROR")
                return
        ...
        if alg_conf["number_of_iterations"] is not None:
            if not self._phasing_algorithms_configs:
    ...
    def _prepare_reconstruction(self):
        ...
        if self._mask is None:
            self._log("You did not set a mask, therefore initializing without masking any pixels \
                       of intensity pattern.", "INFO")
            self._ready = True
        ...
        if self._support_algorithms_configs == []:
            self._log("Reconstruction cannot start! You need to set the support algorithm.",
                      "ERROR")
            self._ready = False
        if self._phasing_algorithms_configs == []:
            self._log("Reconstruction cannot start! You need to set the phasing algorithm.",
                      "ERROR")
            self._ready = False
        ...
        if self._i_support_algorithms is not None:
            if self._i_support_algorithms != self._number_of_iterations:
                self._log("Cannot prepare reconstruction. Support algorithms initialised are not \
                           in line with the set number of iterations.", "ERROR")
                self._ready = False
        if self._i_phasing_algorithms is not None:
            if self._i_phasing_algorithms != self._number_of_iterations:
                self._log("Cannot prepare reconstruction. Phasing algorithms initialised are not \
                           in line with the set number of iterations.", "ERROR")
                self._ready = False
        if not self._ready:
            return
    ...
    def _init_iterations(self):
        if not self._iterations_dirty:
            ...
            return
        self._out_iterations_images = np.linspace(0, self._number_of_iterations, self._number_of_outputs_images).round().astype(np.int64)
        self._out_iterations_scores = np.linspace(0, self._number_of_iterations, self._number_of_outputs_scores).round().astype(np.int64)
