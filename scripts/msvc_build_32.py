...
if(not os.path.exists('C:\msys64') and not os.path.exists(msys2_exe)):
...
if(not os.path.exists('c:\Program Files (x86)\Microsoft Visual Studio 11.0') and not os.path.exists(vs_exe)):
...
if(not os.path.exists('c:\Program Files (x86)\HDF_Group/HDF5') and not os.path.exists(hdf5_zip)):
    geturl(hdf5_url,hdf5_zip);
    file = zipfile.ZipFile(hdf5_zip, "r");
    file.extractall("c:\hdf5");
    os.system("c:\hdf5\HDF5-1.8.15-win32");
...
if(not os.path.exists('c:\fftw3-32') and not os.path.exists(fftw_zip)):
